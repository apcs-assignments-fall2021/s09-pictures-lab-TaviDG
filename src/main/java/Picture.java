...
public class Picture extends SimplePicture
{
    ...
    public void grayscale() {
        ...
        for (int row = 0; row < pixels.length; row++) {
            for (int col = 0; col < pixels[0].length; col++) {
                Pixel pix = pixels[row][col];
                int avg = pix.getBlue() + pix.getGreen() + pix.getRed();
                avg /= 3;
                pix.setRed(avg);
                pix.setGreen(avg);
                pix.setBlue(avg);
    ...
    public void fixUnderwater() {
        ...
        for (int row = 0; row < pixels.length; row++) {
            for (int col = 0; col < pixels[0].length; col++) {
                Pixel pix = pixels[row][col];
                pix.setGreen(pix.getGreen()-70);
                pix.setBlue(pix.getBlue()-45);
                pix.setRed(pix.getRed()+60);
    ...
    public void mirrorDiagonal() {
        ...
        for (int row = 0; row < pixels.length; row++) {
            for (int col = 0; col <= row; col++) {
                ...
                Pixel pix2 = pixels[row-distance][row];
                pix2.setBlue(pix.getBlue());
                pix2.setRed(pix.getRed());
                pix2.setGreen(pix.getGreen());
